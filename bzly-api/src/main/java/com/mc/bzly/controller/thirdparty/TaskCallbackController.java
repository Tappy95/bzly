...
public class TaskCallbackController extends BaseController{
	...
	@Reference(version = WebConfig.dubboServiceVersion, interfaceClass = TaskCallbackService.class, check = false, timeout=10000)
	private TaskCallbackService taskCallbackService;
	
	@NeedAuth
	@RequestMapping(value="list",method=RequestMethod.GET)
	void queryList(HttpServletRequest request,HttpServletResponse response,TaskCallback taskCallback){
		Map<String, String> respMap = new HashMap<>();
		initResponseMap(request, respMap);
		
		Result result = new Result();
		try {
			Map<String, Object> data=taskCallbackService.selectList(taskCallback);
